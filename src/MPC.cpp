...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    const int velocity_mult_factor = 1;//10;

    const int delta_steering_mult_factor = 100;
    const int delta_velocity_mult_factor = 1; //10;


    for (uint i = 0; i < N; i++) {
      fg[0] += cte_mult_factor*CppAD::pow(vars[cte_start + i] - ref_cte, 2);
      fg[0] += epsi_mult_factor*CppAD::pow(vars[epsi_start + i] - ref_epsi, 2);
      fg[0] += CppAD::pow(vars[v_start + i] - ref_v, 2);
    }

    for (uint i = 0; i < N - 1; i++) {
      fg[0] += steering_mult_factor*CppAD::pow(vars[delta_start + i], 2);
      fg[0] += velocity_mult_factor*CppAD::pow(vars[a_start + i], 2);
    }
    for (uint i = 0; i < N - 2; i++) {
      fg[0] += delta_steering_mult_factor*CppAD::pow(vars[delta_start + i + 1] - vars[delta_start + i], 2);
      fg[0] += delta_velocity_mult_factor*CppAD::pow(vars[a_start + i + 1] - vars[a_start + i], 2);
    }
    ...
    for (uint i = 0; i < N - 1; i++) {
      ...
      // polynomial therm of 3rd free
      AD<double> f0 = coeffs[0] + coeffs[1] * x0 + coeffs[2]*x0*x0 + coeffs[3]*x0*x0*x0;
      // derrivative of the polynominal therm
